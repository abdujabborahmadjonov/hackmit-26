  ...
  return (
    <div>
      <PageHeader
        ...
        description="Share syllabi, homework and lesson plans, or browse what other educators have uploaded — ranked by how well they fit your teaching context."
      />

      <div className="mt-7 flex gap-1 overflow-x-auto rounded-xl bg-slate-100 p-1">
        {tabs.map(({ key, label }) => (
          <button
            key={key}
            onClick={() => setTab(key)}
            className={cx(
              "shrink-0 rounded-lg px-5 py-2 text-sm font-medium transition",
